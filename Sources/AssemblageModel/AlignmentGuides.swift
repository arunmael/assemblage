...
public enum AlignmentGuides {
    ...
    private static func axisCandidates(
        ...
        candidates += edgeAndCenterCandidates(
            ...
            axis: axis,
            // Die Leinwand ist kein Objekt, an das man etwas anlegt: „linke
            // Kante an rechte Leinwandkante" würde die Ebene vollständig neben
            // die Leinwand rasten — und zwar genau dann, wenn man sie an den
            // Rand schiebt.
        ...
        for other in otherFrames {
            ...
            candidates += edgeAndCenterCandidates(
                ...
                axis: axis,
                // Zwischen zwei Ebenen ist die gegenüberliegende Kante genau
                // das, was man will: bündig aneinanderlegen.
                includesOppositeEdges: true
            )
        }
    ...
    /// Erzeugt die Kandidaten für Kantenausrichtung und Zentrieren (Mitte an Mitte)
    /// zwischen der gezogenen Ebene und einer Referenz (andere Ebene oder Leinwand).
    ///
    /// `includesOppositeEdges` schaltet das bündige Anlegen (linke Kante an rechte
    /// Kante) zu. Bei anderen Ebenen ist das erwünscht; bei der Leinwand nicht,
    /// weil die Ebene dabei vollständig ausserhalb landen würde.
    ///
    ...
    private static func edgeAndCenterCandidates(
        ...
        var edgePairs: [(Double, Double)] = [
            (dragged.low, reference.low),    // gleiche Kante (z. B. links an links)
            (dragged.high, reference.high)
        ]
        if includesOppositeEdges {
            edgePairs += [
                (dragged.low, reference.high),   // bündig anlegen
